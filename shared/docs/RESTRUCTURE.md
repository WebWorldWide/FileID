...
Status as of 2026-06-16: **P1 + R1 (non-image extension) built + verified on BOTH engines
(macOS `swift test` 218/218, Rust `cargo test` 343/343 + clippy clean). P2 (VLM naming) and the
P3 corrections/undo follow-ons remain. On-hardware butler-quality verification + non-image
threshold tuning pending (owner UAT).** See `STATE.md` / `NEXT.md`.

- **P1 — engine: semantic + learn-your-style classify. ✅ Done (both engines).** `pipeline/restructure_semantic.rs` (+ Swift mirror): fuse signals → cluster (reuses `identity_clustering`) → folder-prototype assignment with confidence/abstain. Unit-tested; rule cascade is the fallback.
- **R1 — non-image extension. ✅ Done (both engines).** P1 ran on images only; R1 adds an *additive* filename+tag bag-of-words pass (`classify_non_image`, separate tighter `nonImageProfile`) so documents/video/audio cluster by content too, while the image path stays byte-identical. Generic dumping-ground folders are barred from being learn-your-style prototypes. Env-tunable (`FILEID_RESTRUCTURE_NI_*`, kill-switch `FILEID_RESTRUCTURE_NONIMAGE`); threshold calibration on a real library pending. See `DECISIONS.md` 2026-06-16.
