...
- **[faces/HIGH — the real face fix] mega-merge + fragmentation at scale.** The floor fix removes junk singletons, but a ~44k-face scale test exposed the deeper problem: bridge-face over-merge (an **11,665-face "person"** with ~0.30 median intra-cosine — many different people chained) that `FILEID_FACE_MUTUAL_KNN=1` does NOT break and Pass-3's split cap (7) can't shred, plus size-2/3 fragmentation. This is a clustering-ALGORITHM change (candidates: raise/redesign the Pass-3 recursive split; density/HDBSCAN or Leiden over the kNN graph; a verification-net re-check on merge). **CANNOT be validated on cluster counts alone — needs a small LABELLED face set (ground-truth identities) to measure precision/recall.** Build/borrow that first; don't tune blind on the unlabelled corpus (overfitting risk). See STATE 2026-07-05.
