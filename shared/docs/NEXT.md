...
- **`FileIDSetup.exe` (Burn bundle) not built** — it chains BOTH x64 + arm64 MSIs, so it needs an aarch64 engine cross-build + arm64 self-contained publish (+ WiX bundle). 0.0.1 shipped the x64 MSI only.
