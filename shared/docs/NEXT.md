...
2. **On-hardware follow-ups (this box):** (a) A/B `FILEID_MODEL_POOL_SIZE` 1 vs 4 on a quiet machine — the 71k soak ran pool-1 (transient Low tier from parallel builds) at ~21 img/s; pool-4 measured 60-file scans faster, quantify the delta at scale and whether decode/USB is now the ceiling. (b) Deep Analyze: verify `-np 2` wave parallelism + 1568 px downscale on a real VLM batch (Qwen2.5-VL) — throughput and output-quality spot-check. (c) Full app (WinUI) run on Adlon: welcome-modal install states, "Counting files" → real ETA handoff, no flicker.
