...
2. Check the People six-face disclosure, including a last-name-only cluster;
...
1. **The publication blocker is unchanged and still governs signed release.** Strict runtime egress rejects the same ten reviewed GitHub/NVIDIA archives and the widened redirect allowlist. Provision byte-identical, license-vetted Hugging Face mirrors (or remove those download paths), refresh the digest baseline, and require the strict no-flag gate to pass before anything is signed and published as a real release. DirectML remains the approved Windows GPU path; CUDA/OpenVINO/QNN stay owner-provisioned development paths.
2. **Fix the dev-environment toolchain hazard on the Windows host.** A standalone Rust 1.96 install precedes the rustup shims on `PATH`, so plain `cargo` silently bypasses the repo's 1.90 pin that CI uses. Until the shim wins, verify with `~/.cargo/bin/cargo.exe` explicitly. Mixing the two also poisons a shared target directory (`E0514` cascading into unrelated crates); a full `cargo clean` is the recovery, not a `--target`-scoped one.
3. **Run the external platform/hardware gates.** Native macOS Swift/Rust build/test and model parity; Windows ARM64 and AMD/Intel/QNN; native distro/Flatpak/AppImage installs; clean-VM install/upgrade/repair/uninstall; accessibility; and signing/notarization all remain unverified locally.
4. **Consider two product observations from the real-data run, neither a defect.** The 50 MP decode cap rejects legitimate family panoramas (measured ~60 MP) — raising it trades a decompression-bomb margin for coverage, so measure peak RSS before changing it. Face clustering produced 2,728 persons from 104,958 faces with several very large clusters, which is worth a calibration look against named ground truth before v1.0.
5. **Keep the invariants that are now machine-checked.** `EventHandlerSafetyContractTests` enforces the SafeRun rule and asserts a floor on subscriptions inspected; the reviewed network-capable source digests are a review tripwire that must be refreshed deliberately, never blanket-regenerated.

Evidence: `%TEMP%\tmpnlwju2.tmp\events.jsonl` (full run; `scanComplete` 135,740/195/4,605.6 s, `faceClusteringComplete` 2,728/104,958/51.3 s), the harness log with all 13 assertions and `ITERATE_EXIT=0`, and the post-scan fingerprint matching metadata SHA-256 `529f3a9f9c0cd54e5842bbd347d83f4edb6a62ac28aea5810db712a9c079725b`.
...
2. **On-hardware follow-ups (this box):** (a) DONE — pool A/B measured (1→20.4, 2→23.4, 3→17.2, 4→4.6 f/s on identical 3000-file runs); CUDA pool now defaults to 2 (`CUDA_MODEL_POOL_MAX`). DONE TOO — DCT-scaled decode for oversized JPEGs shipped default-ON after a controlled A/B proved it equivalent (see DECISIONS 2026-07-21): 3000-file subset 128-147 s → 84.8 s (~35 f/s). Follow-up: adopt the same decode policy in the macOS engine for cross-platform output parity; until then set `FILEID_JPEG_SCALED_DECODE=0` for byte-parity corpus comparisons. (b) Deep Analyze: verify `-np 2` wave parallelism + 1568 px downscale on a real VLM batch (Qwen2.5-VL) — throughput and output-quality spot-check. (c) Full app (WinUI) run on Adlon: welcome-modal install states, "Counting files" → real ETA handoff, no flicker.
...
3. Clean-VM matrix (item 3 below) extended with major-upgrade **while a scan is mid-flight**: `FileIDEngine.exe` is a windowless child process holding the SQLite WAL that RestartManager doesn't see; the engine is kill-tolerant by design, but the prompt/reboot/post-upgrade-DB behavior is unverified on hardware. `Product.wxs` header documents the case.
4. `Product.wxs`: StartMenuShortcut component keypath moved **HKCU→HKLM** — an HKCU keypath in a perMachine package (ICE38/ICE57 shape) strands the value when a different user uninstalls and can trigger repair prompts in multi-user setups. Auto-GUID changes with the keypath; fine across MajorUpgrade (old product removed first).
5. `windows-app.yml` startup smoke: **any** exit within 5 s now fails (a startup crash can exit 0; the old gate passed it). The runner has an interactive desktop, so a healthy build stays alive. If the PR run reveals a legitimate headless fast-exit, gate it explicitly instead of re-allowing exit 0.

Audit notes for the record: `async void` sites are all event handlers; downloader verifies SHA-256 before rename/extract; trash/restructure carry kill-tolerant journals + reparse TOCTOU re-checks; OneDrive placeholders are never hydrated; EngineClient has bounded 1s/4s/16s backoff + 3-strike terminal handling. Not exhaustively re-covered this pass: a full three-way IPC drift diff and a per-ViewModel status-lie sweep (the conformance suites cover the contract in CI). Next step: branch, PR, confirm windows-app/windows-engine workflows green (the smoke-gate change in #5 is deliberately load-bearing on that run).

## STATUS 2026-07-16 — LANDED: cross-platform safety + macOS Deep Analyze tree merged to `main` CI-green (PR #106); v0.1.1 refreshed

The large uncommitted tree (macOS Deep Analyze/VLM/CLIP port, model-license gating, archive path-safety, engine-lifecycle safety, read-only fingerprinting) is **MERGED** (PR #106) with all 18 CI checks green, and the **v0.1.1 prerelease was refreshed in place** — the tag was moved to the merge commit, `release.yml` rebuilt every artifact, and all 10 assets were re-uploaded (2026-07-17). A 3-round adversarial audit (~50 agents, converged clean) fixed **11 defects** (STATE top); a read-only Adlon scan proved the engine byte-identical-on-corpus.

**CI hardening (the mid-flight tree was never CI-tested; `main` was green, so every failure was self-introduced and resolved across 11 commits):** macOS CI was bumped to a **macos-26 / Xcode 26** runner (owner-approved) because the code targets the Xcode 26 Swift 6.3 SDK and Xcode 16 rejects its Sendable conformances (`NSImage`/`CIImage`/`mach_task_self_` → the engine now reads resident memory via `proc_pidinfo`). Also fixed: Linux `gtk::Dialog` deprecation + a `clone!`/`#[weak]` E0069 + 3 clippy lints, .NET CA1068 + UTF-8 BOM on 5 new `.cs` files, the quinn-proto CVE, and the model-license terms hosts (`ai.google.dev`, `docs.nvidia.com`) allowlisted in all three privacy scans (bash array in `macos.yml`, PowerShell in `windows-*`). See the `ci-toolchain-and-logs` memory.

Remaining follow-ups:

1. **Owner runtime-verify (Windows, can't run here):** (a) the mutation-gate change — pause a scan, then issue a mutation, and confirm a `library_busy` error (not a hang); (b) the license gate now actually installs the CUDA/cuDNN pack + Gemma on a shipped unpackaged build (was 100% blocked pre-fix). Known fail-safe edge (LOW): a scan queued behind a long op when Wipe is clicked → wipe times out "nothing wiped" and succeeds on retry (it never truncates during a scan).
2. **Linux polish (deferred, `#[allow]`-suppressed to land):** migrate `model_license.rs`'s deprecated `gtk::Dialog` to `adw::AlertDialog`; wire or remove `cleanup.rs::recompute_group` (currently `#[allow(dead_code)]`).
3. **[CLI/engine parity, deferred] truncated-to-0-byte files** keep their stale row/metadata on both the engine and the CLI (seen-but-not-catalogued). Re-indexing them as empty is a cross-platform change (engine + CLI together), not a CLI-only fix — do it as a deliberate parity change if desired.
...
1. **Mac pass (Swift changes are written and statically reviewed — compile + test + validate on a Mac):** (a) the iteration-14 Cleanup fixes (ReadStore group-ranked candidates, CleanupView scan-aware exact reload, EngineClient App-Nap tokens) — `swift build && swift test`, then real Cleanup UI behavior; (b) face auto-merge threshold ordering (`tight` 0.65 < `pass1` 0.66), mutualKNN default, stale ArcFace docs — recalibrate mirroring the Windows re-cluster sweep method (evidence log has the recipe); (c) the 250k embedding-row auto-merge preflight cap (confirmed 3/3): re-scope to real cost drivers or surface an explicit "auto-merge skipped" event; (d) the earlier deferred Intel-ORT/Whisper-row/menu-disabled items.
2. **Remaining stability items (engine):** process-lifetime gpu_dead latch (per-scan today; app has no gpu_device_removed handler); mid-scan USB-disconnect regression for the iterate harness; EngineClient stale-sender/_expectingExit ordering + Apply() SpawnGeneration check (app-side). Throughput: CUDA is ALREADY active on the dev box (2026-07-15 log-confirmed EP chain + Settings), and the 71k Adlon soak is disk-bound on the USB SSD (~440 MB/s), so the EP is not the ceiling for large-media corpora — the remaining compute-side lever is a dynamic-batch RAM++ export (the batch coordinator is inert without it), which mainly helps smaller/faster-disk libraries. Re-scan Adlon after the face-align OOB fix to regenerate the ~1.1% smeared crops.
3. **Restructure post-P1 design items:** face/person block in semantic fusion (§2), hierarchy/label-merge to shrink the ~2,260-category wall, VLM group naming (P2), content-cohesion anchors (category-homogeneity exempts giant photo dumps), person-route fixes in the cascade (alphabetical-first name, coordinate `Places/` folders), truncated-plan review UI (app hides Sankey/per-file review above the preview cap).
4. **Faces follow-ups:** name-aware clustering guard (Pass-1 can fuse two named people; majority-vote persist silently drops the loser) + stable person identity across rescans (DROP+rebuild loses names on churn) — design sketch in the ledger; verdict bbox-key robustness (exact-float JSON join).
5. Then the prior release gates below (mirrors, signing provider, clean-VM matrix, CI landing) — unchanged.
...
1. **Mirror the six release-blocking Windows runtime archives.** Upload byte-identical, license-vetted llama.cpp Vulkan/CUDA, Whisper, cuDNN, CUDA runtime, and ONNX Runtime CUDA archives to the approved Hugging Face organization; retain their reviewed SHA-256 values, replace every registry URL, and reduce the downloader allowlist to `huggingface.co`/`hf.co`. Acceptance: `python shared/scripts/check_runtime_egress.py` passes strict mode and the known-blocker baseline is empty/removed. Current development inputs are locked against additions, but signed publication intentionally fails before staging.
2. **Choose/onboard a public-trust signing provider.** Follow `WINDOWS_SIGNING.md`; require a project-specific independently pinnable key identity, protected human-approved environment, OIDC/provider auth, and one signer across every FileID PE/MSI/Burn layer. The separate output-gated `contents: write` publication job now exists; provider authentication must be added to make the signing job set `publish=true`. Acceptance: real signatures + timestamp chain pass the common verifier, app rejects a wrong-key engine fixture, Windows native tool archives are signed before packaging, and a disposable prerelease tag publishes only verified staged assets. Current tag workflow remains unsigned validation-only until provider enrollment; manual non-tag publishing remains blocked.
3. **Clean-VM installer matrix.** On Windows 10 22H2, Windows 11 x64, and Windows 11 ARM64: install prior 0.1.0, major-upgrade to 0.1.1, repair a deleted payload, uninstall, inject one package failure to verify rollback, and confirm ARP/shortcut/cache/user-data behavior. Run keyboard-only, Narrator, high-contrast, and 200% scaling checks. ARM64 build/UI was not produced on this x64 pass. Also: major-upgrade **while a scan is mid-flight** (app + engine running) — expect a files-in-use/RestartManager prompt, no reboot-required surprise, and a sane post-upgrade DB; `FileIDEngine.exe` is a windowless child holding the SQLite WAL that the installer doesn't know about (kill-tolerant by design, unverified on hardware — see Product.wxs header).
...
5. **Preview resource inspection.** UIA proves image render/close/reopen. Use Process Explorer/handle tooling with image, video, and audio previews to confirm file handles/media playback release after close; repeat on installed signed bits.
6. **Land the branch and run GitHub CI.** Review the final diff, commit `windows-release-polish`, merge to `main`, and require every engine/app/tools/macOS/Linux workflow green before tagging.
...
> **History boundary:** every dated `STATUS` section below is a preserved session snapshot, not current release identity or current priority order. The 2026-07-12 section above is authoritative.
...
- **`FileID.Msi.wixproj` version-gate is broken for local builds.** `VerifyEngineVersionMatchesVersionFile` (line 88) parses `src/engine/Cargo.toml` version as `''` under both `dotnet build` and VS18 msbuild → fails with a false "Version drift" even though VERSION and Cargo.toml both say `0.1.0`. Blocks `publish-bundle.ps1` / any local MSI build. Was worked around this session by temporarily guarding the `<Error>` on empty-parse (reverted). Fix the `ReadLinesFromFile` + `%(Identity)` regex batching so the parse works, or guard on empty-parse permanently.
- **Version mismatch:** internal product `VERSION` = `0.1.0` but the release is tagged `v0.0.1` (owner's chosen release name). Align `VERSION` + engine `Cargo.toml` to `0.0.1` if the tag should match the product version.
- **`FileIDSetup.exe` (Burn bundle) not built** — it chains BOTH x64 + arm64 MSIs. ATTEMPTED 2026-07-05: the aarch64 engine cross-build fails on this box with `E0463 can't find crate for core/std` on every crate even though `rust-std` reports installed and `libcore-*.rlib` exists at rustc's `--print target-libdir` — a rustup toolchain-state corruption (`rustup run 1.90-x86_64 cargo build --target aarch64-pc-windows-msvc` still fails). Fix needs a full toolchain reinstall (`rustup toolchain uninstall 1.90 && rustup toolchain install 1.90 && rustup target add aarch64-pc-windows-msvc`), then arm64 self-contained publish + WiX bundle. NOT worth it for 0.0.1: the **x64 MSI already is** the single-file double-click installer for x64 Windows (99%+ of users); the bundle only adds ARM64 + a `.exe` extension. Revisit if ARM64 Windows demand appears.
...
- **[faces/DONE — label-validated] clustering retuned to F1 1.0 on the owner's labels (PR: faces-label-retune).** The mega-cone fix (pass1 0.66→0.82, cohesion-only) was superseded once the owner labelled ~185 faces: labels showed 0.82 was FAR too high (recall ~1%). Shipped `pass1=0.50`, `pass2=0.45`, mutual-kNN default-ON, and a new `FILEID_FACE_CLUSTER_MIN_QUALITY=0.35` pre-clustering gate → labelled People-tab precision/recall = 1.0 (STATE 2026-07-05). **Remaining:**
  - **Cross-corpus generalization:** the thresholds are calibrated on ONE (low-quality, scanned) corpus. Collect a few labelled faces on a MODERN phone-photo library and confirm 0.50 doesn't over-merge lookalikes there (same-person is higher, ~0.85+, so it should hold; verify).
  - **Cross-age + very-low-quality faces are an EMBEDDING ceiling, not a clustering one** — SFace can't match child↔adult or noise faces. The only real lever is a stronger face embedder (bigger ArcFace) — a model/licensing decision, not a threshold. Documented, deferred.
  - **Face-labeler tool** lives at scratchpad `make-labeler.py` (DB + crops → self-contained HTML → `labels.json`); consider promoting it into `platforms/windows/build/` for repeatable calibration.
  - Full-corpus (84k) confirmation once the F:\TrueNAS scan can finish (`-ScanTimeoutMinutes 120`; external drive is slow).
- **Confirm at true full scale:** the 44k numbers came from a *timed-out partial* scan (Adlon external drive ran <17 f/s, hit the 60-min iterate cap). Re-run the full 62,731-file scan with `-ScanTimeoutMinutes 120` (and less concurrent load) to get the real 84k-face post-fix count.
...
- **[perf/MED] RAM++ dynamic-batch ONNX re-export — now unblocked.** `onnx 1.22` has a Py3.14 wheel (the old blocker is gone). Because the 5080 is dispatch-bound not compute-bound, batched RAM++ (fewer, larger dispatches) could help where pooling (more dispatches) hurt — the opposite of the 2060 result. Needs `export_ram_plus_onnx.py --dynamic-batch` against the `ram_plus_swin_large_14m.pth` checkpoint (HF: xinyu1205/recognize-anything-plus-model), then A/B via `measure_batch.ps1`, then MODELS.md vetting + SHA-pin before shipping.
- **[DONE 2026-07-04] iterate.ps1 + perf-script retune** — `-ThroughputTarget 100→25`, `-MemoryCapMB 6000→8500` (both to measured DirectML reality); stale `G:\TrueNAS` → `F:\TrueNAS` in the 5 live perf scripts. Pool constants left unchanged (measured-optimal on both cards).
...
Findings from the M11 endgame adversarial audit (2026-07-04, engine + Linux app, verified):
- **[engine, FIXED — DATA LOSS] rename-heal ON CONFLICT REPLACE clobber** — the heal's plain `UPDATE path_text` silently REPLACE-deleted a live copy's row + user tags/person names (guard was dead code). Fixed with `UPDATE OR ABORT`; empirically proven + regression test.
- **[engine, FIXED — DATA LOSS, sibling] restructure_apply + bulk-rename `UPDATE path_text`** had the same clobber bug as the heal, reachable under a transient update failure mid-restructure (Windows), a non-Windows check→rename TOCTOU, or external DB/disk desync — empirically verified. Both now `UPDATE OR ABORT`, routing the collision into each site's existing recovery arm (record_path_update_failure / record_rename_recovery) instead of REPLACE-deleting a live row. Regression test added. A dedicated verification pass confirmed `files.path_text` is the ONLY `UNIQUE ON CONFLICT REPLACE` column (blast radius fully bounded to these 3 sites + the safe named-target UPSERT) and that the heal fix is complete (no half-heal; path_search is non-unique).
- **[engine, FIXED] CLIP encoders lacked output-dim guard** — image+text, all 4 forward paths; now bail on width != 512 (mirrors SFace/RAM++/BGE).
- **[linux/gtk, FIXED] restructure Undo armed with wrong root after destination switch.**
- **[engine/test, LOW] priority_chain tests `remove_var("FILEID_GPU_EP_OVERRIDE")`** but production reads the override from app-settings JSON, so the isolation is a no-op — a dev box with `gpuExecutionProviderOverride` set false-fails these tests (CI is fine, fresh checkout). Fix: point tests at a temp `app_settings_path()` (like the ep_guard tests). Not a runtime bug.
- **[linux/gtk, LOW] `people.rs` thumb_cache unbounded** (no eviction, unlike Library's FIFO-512) — bounded by person count so tens–low-hundreds of MB ceiling; add a cap for parity.
- **[linux/gtk, LOW] engine_client respawn stops on a *spawn* failure** (only `Exited` drives respawn; a failed respawn-spawn goes quiet without hitting the "giving up" cap). Also `drain_stdout` has no line-length cap (info-only; engine is trusted + newline-frames).
- **[linux/gtk, LOW] Settings download poll timer** runs indefinitely on a stalled download (only breaks on complete/cleared).
- **[engine, PARITY?] `has_text` set only in the image-OCR block** — text-bearing docs/PDFs store `has_text=0` (cosmetic: Library text badge won't show on doc tiles; FTS + restructure unaffected). Confirm against macOS reference.
- Two suspicions DISSOLVED on inspection: Linux Cleanup per-group delete has no confirm but **mirrors macOS by design** (CleanupView.swift:21) — a Linux-only confirm would break the 1:1 parity mandate, so left as-is; the face over-clustering (10,208 persons / 84,629 faces) is a **deliberate fail-toward-over-split calibration** (mergeable in UI), not a logic bug.
...
**On-hardware follow-ups discovered this session (highest priority):**
- **[app/MED] Welcome/install sheet shows every launch** — `MaybeShowWelcomeSheetAsync` sentinel check keys on `clip`/`arcface`, but the installed commercial-clean models are `mobileclip`/`sface` (stale sentinel names after the model swap). The app reports `clip=NotInstalled arcface=NotInstalled` despite the models being present. Find the sentinel-name mapping (likely `ModelInstallerService`/`SlotFor`/sentinel paths) and align it to the shipped model set. Verify with a fresh app launch (welcome sheet should NOT appear when models are installed).
- **[app/LOW] `PeopleView.OnLoadedAsync` catch logs empty `ex.Message`** — `RefreshAsync` swallows its own exceptions, so the escaping throw is an `OnUi` teardown race (benign, fires on app-close). Log the full `ex` (type+stack) there to make it diagnosable, and consider catching `OperationCanceledException` separately so app-close races don't WARN. (`PeopleView.xaml.cs:96`.)
- **[engine/perf, NOW PROFILEABLE] peak RSS 3635 MB > 1500 MB cap** — the cap is macOS-MLX-derived; the Windows ONNX/CUDA stack baseline is ~2.4 GB just for loaded models (measured: 2377 MB on a 3-file scan = pure model load). Profile with `build/profile_gpu.ps1`/`measure_batch.ps1` to decide: raise the Windows cap, or find reducible per-scan growth (the perf agent flagged RAM++ batch-path full-frame clones + decode-pool buffers). On-hardware now possible.
- **[engine/perf, NOW PROFILEABLE] ndarray preprocessing linear-fill** — deferred earlier for lack of hardware; now the box can A/B it. Add a byte-identity golden test, apply the planar `chunks_exact` fill, and measure the actual throughput delta vs the ~7.9 f/s GPU-bound baseline before keeping.
- **[CLI/TUI/MED] remaining audit items** — CLI: exit non-zero when `result.failed>0` (apply currently returns 0); `--exact --similar --apply` silently drops `--similar`. TUI: render-loop dirty-flag (redraws ~10×/s idle + `visible_files()` allocates per frame), panic hook to restore the terminal before printing. All headless-verifiable in their workspaces.

**Cleanup from this session:** restore the user's `face_crops` via a rescan (they were wiped during testing, not backed up — DB face data is intact); the `%LOCALAPPDATA%\FileID-backup-onhwtest` DB backup can be deleted once the user confirms their library is fine; stray test artifacts (`%TEMP%\fileid-ocrtest`, `build/smoke-out/`, the two repo-root junk `.ps1` files).

**Original engine robustness backlog (still open, headless-verifiable):**

**Commit/verify gate (do first):**
- Commit the 11-file engine changeset on a branch + push, confirm the engine CI workflow is green, then merge. Then **on-hardware** (RTX 2060 / TrueNAS, `build\iterate.ps1`): re-verify OCR still produces text after the STA→MTA change (`shell/ocr.rs`), and a full scan + restructure plan still behaves (the `restructure_semantic` shared-registry threading + dbwriter `DELETE…RETURNING`).

**Rust — verifiable headless (do next):**
- **[robustness/MED]** `downloader.rs` — add a runtime initial-URL host allowlist (mirror `REDIRECT_ALLOWED`; today only redirect hops are checked — egress policy is test-only, not enforced in code); assert `actual_len == total` post-concat for hash-less files (`download_parallel`/`download_simple`); cap per-stream bytes at the advertised length (disk-fill backstop). Security-relevant; careful pass.
- **[robustness/MED]** `main.rs` stdio loop — inline-dispatched arms + the `from_str` decode have no panic firewall; a panic unwinds `stdio_loop` (never `.await`ed) → silent hang holding the DB writer, no error frame. Wrap per-line dispatch in `catch_unwind`/`spawn().await` + emit an `error` frame, or detect loop completion and signal shutdown. (Latent — no current inline arm panics.)
- **[perf/MED, needs hardware]** ndarray per-pixel preprocessing (`ram_plus`/`mobileclip`/`sface`/`yunet`) — replace 4-D strided `chw[[0,c,y,x]]` writes with a contiguous planar fill (`as_raw().chunks_exact(3)` → `Array::from_shape_vec`). Provably byte-identical by construction, BUT a transcription slip = silent ML-quality regression with no golden test. **Gate on: (1) add a byte-identity golden test old-vs-new, (2) on-hardware profile to confirm the gain isn't swallowed by GPU-bound inference (RAM++ ≈670ms/file).**

**C# — needs `dotnet build`/`test`/`format` (CI/Windows):**
- **[perf/HIGH, sensitive]** `DebugLog` is not level-gated; `Debug`/`Info` do synchronous locked unbuffered file I/O on the UI thread in scroll (`[THUMB]` per tile) + scan (`[APPLY:N]` per event) hot paths. Add a runtime level gate so verbose `Debug` lines (and their `PathRedactor.Redact` arg-eval, which must be guarded at call sites) compile out at `Info`. **Load-bearing forensic infra** (CLAUDE.md: keep `[APPLY:N]`/`[ENGINE-SUB]`, do NOT re-batch) — confirm which lines are gateable Debug vs mandatory Info, and hardware-verify the next native fast-fail still gets captured.
- **[perf/MED]** `LibraryViewModel.MergeById` reorder is O(n²) (`IndexOfInstance` per item) — build a `Dictionary<FileTile,int>` once or fall back to a single `ReplaceAll` Reset when the permutation distance is large.
- **[perf/LOW]** `FileTile.DateDisplay`/`SizeDisplay` recompute+allocate per binding read — materialize once in `FileTile.From` (match the `TopTwoTags` precedent). `Convert.ToInt32`→`ToInt64` on COUNT/SUM reads (6 sites; contained by try/catch, >2.1B-row overflow only — trivial hardening). `ReadStore.RecentAsync`/`SearchAsync` per-row `GROUP_CONCAT` subquery — confirm an index on `tags(file_id, source, score)` exists (engine migration); else batch the tag fetch.
- **[perf/MED, latent]** `Theme/IridescentBorder` allocates an array+brush+4 dict lookups every vsync + never pauses when occluded — but the control is **unused** (no XAML/code refs). Fix the per-frame alloc + add `XamlRoot.IsHostVisible` gating (mirror LavaLamp) before it ships, or note the doc/impl mismatch (`CanvasSweepGradient` claimed, `CanvasRadialGradientBrush` used).
- **[robustness/MED]** `Theme/CompletionRipple` — Popup/Storyboard leak if the target unloads mid-animation (only `sb.Completed` closes it); add defensive teardown. `SankeyFlowControl` has no `Unloaded` → `_renderDebounce` timer keeps a detached control alive ≤80ms; stop it on Unload.

**Coverage gaps for a future round:** Swift macOS (recently audited 2026-06-23, re-verify only if changed); more C# Views (only EngineClient/ViewModels deeply read); the prior backlog below (some items overlap — e.g. `restructure_semantic` per-segment clone, discovery `has_build_marker`, ORT session-bind boilerplate). **Cleanup:** delete the two stray untracked junk files (`platforms/windows/test_vuln.ps1`, `C：Users…keytest.ps1`).

**Honest ceiling:** the remaining HIGH-value perf wins (preprocessing, DebugLog) and ALL behavior/threshold tuning need on-hardware profiling + the WinUI runtime + the TrueNAS corpus — they cannot be completed or proven from the headless dev box. Source-level correctness/robustness is what's tractable here.
...
- **Engine-spawn command IPC** — drive live `scan`/`cluster` by spawning `FileIDEngine` and reading newline-JSON `IpcEvent`s, rendering real progress on the status line. The plumbing is pre-wired: a background thread already feeds a `LoadMsg` `mpsc` channel; add a stdio event reader that maps `IpcEvent` → `LoadMsg::Status`/reload. Mirror `platforms/linux/.../engine_client.rs`. Acceptance: pressing a "scan" key in-app indexes a folder with live per-file progress and the lists refresh on completion; degrades to the CLI's `models_not_installed` notice when models are absent.
- **In-app scan/cluster actions** — key bindings (e.g. `s` scan path prompt, `c` recluster) once the IPC feed lands. Until then the workflow is CLI `scan` → `r` reload.
- **Restructure apply (guarded)** — the plan is currently a read-only preview; wire `applyRestructure`/`undoRestructure` behind an explicit confirm modal + dry-run default, sharing the CLI's apply path when that lands.
...
The cross-platform CLI (`platforms/cli/`) is verified green on the macOS host (clippy `-D warnings`, build, test). The read/plan MVP plus the apply + full-pipeline + similarity follow-ons have landed. Remaining items, roughly in priority order:

- **`fileid apply` (destructive commands) — ✅ LANDED (restructure + dedupe).** `restructure --apply [--dry-run] [--symlinks] [--yes]` runs the engine's exact `RestructureApply` in-process (collision-uniquify, stale-plan + path-traversal guards, undo journal); `dedupe --apply [--similar] [--dry-run] [--delete] [--yes]` removes via the engine's exact `shell::trash::trash` (recoverable on Win/Linux) or permanent `--delete`. SAFE defaults (no apply without the flag; prompt unless `--yes`; non-interactive abort). **Still open:** `rename --apply` (→ `renameFiles`), `retag` (→ `applyTags`), and CLI `undoRestructure` / `restoreFromTrash` reversals. macOS has no engine trash → `dedupe --apply` there needs `--delete`; a freedesktop/macOS trash for the engine is the cleaner fix.
- **Semantic / `--similar` search — ✅ LANDED (visual NN).** `search --similar <path-or-id>` ranks by cosine over `clip_embeddings` (LE-f32 decode), with distinct messages for "no embeddings" vs "seed not embedded". **Still open:** free-text semantic query (`embedTextQuery` → CLIP text encoder) — needs in-process ORT or an engine round-trip, deferred with the rest of model-loading.
- **Full-pipeline `scan --models` — ✅ LANDED.** Mirrors the engine's `startScan` model gate (`mobileclip_s2` + `arcface` sentinels), then spawns `FileIDEngine` and streams its NDJSON IPC (reusing `ipc::IpcCommand`/`IpcEvent`). Missing models / binary → actionable message. **On-hardware gap:** the actual spawn→full-scan path can't be exercised on this Mac (no models, and the test forces the no-models branch); verify on a host with models + a built engine binary that `scan --models` produces the same `tags`/`face_prints`/`clip_embeddings`/`content_hash`/`phash` rows a desktop scan would.
- **TUI (ratatui) — ✅ MVP LANDED** at `platforms/tui/` (`fileid-tui` binary; ratatui+crossterm justified in DECISIONS). Interactive five-tab terminal UI on the same in-process read/classify calls: Library (searchable list + detail), People, Cleanup, Restructure preview, Settings; keyboard nav + live `mpsc` status stream; clippy/-D-warnings + build + 24 tests green on macOS; `tui` CI job in linux.yml. Remaining follow-ons below.
...
GPU Performance Packs, main's own audit sweeps) — fixes re-applied onto the replacement code
where a subsystem changed. **macOS `swift build` is green; everything else needs hardware
...
- Windows: `planRestructure` SQL prepares against bundled SQLite (H10, cargo); restructure
  ...
  added); OCR returns text from a known-text PNG via spawn_blocking (H9 — main's six-workflow
  sweep fixed the same missing COM init; confirm the merged path keeps both the init and the
...
**Data migrations to run after deploy:** H2 (UPSERT) changes re-scan semantics — validate on a
real macOS DB. (H7's "recompute Windows `arcface_embeddings`" is MOOT post-merge: main replaced
Windows ArcFace with YuNet+SFace — `models/arcface.rs` is gone and the v12 migration already
rebuilt the embedding store. Re-verify channel order in the `sface.rs` preprocessing instead.)
...
- ~~L1 — IPC ID-casing drift~~ **RESOLVED by main's PR #6** (`9f1ac52` aligned the capital-`ID`
  casing across Rust+C#+schema, ~25 fields). Residual: confirm with a real Windows app↔engine
  run post-merge (per-language unit tests won't catch a cross-wire slip). See DECISIONS.md.
...
# Carried from main (2026-06-04) — Windows log, still-open items

## 2026-06-04 — Review + land the SIX-workflow deep audit fixes (landed as `4ff9789`)

A second, larger adversarial sweep (6 serialized workflows: engine·app·perf·security + re-audit + regression-repair re-audit) fixed **~35 bugs** in the `main` working tree (layered on the prior uncommitted sweep), and its re-audit caught + repaired **4 regressions its own fixes introduced**. All headless-green (engine clippy `-D` + tests incl. +3 new; app build 0/0 + App.Tests + IpcSchema.Tests + format). **Committed as `4ff9789`** — see STATE 2026-06-04 (latest) + DECISIONS 2026-06-04 (latest) + per-finding record `shared/docs/audit-2026-06-04c/`. Resume order:

1. ~~Review the working-tree diff (39 files) and (owner) commit + push to `main`~~ **DONE — landed as `4ff9789`**; still confirm both GitHub workflows (windows-engine, windows-app) green. The diff was the prior uncommitted sweep + this one combined.
...
## 2026-06-04 (latest) — Review + land the five-workflow bug-audit fixes (folded into the entry above — its fixes landed in the same `4ff9789` diff)

A fresh exhaustive bug-audit sweep (4 audit workflows + a re-audit/critic) fixed **11 bugs** in the `main` working tree (10 files: 4 app + 6 engine), all headless-green (engine clippy `-D` + **267 tests** + fmt; app build 0/0 + **131 + 38 tests** + format). **Committed in `4ff9789`** — see STATE 2026-06-04 (later) + DECISIONS 2026-06-04 (later) + the per-finding record in `shared/docs/audit-2026-06-04b/`. Resume order:
...
3. **Deferred (real, out of this pass's scope):**
   ...
   - **IPC forward-compat / determinism hardening** (no active drift today, so deferred): an unknown enum value drops the whole event (`IpcCoder.cs`); a missing REQUIRED field is silently zero/null-filled (`Dtos.cs`); the schema documented alphabetical key ordering but the engine emits insertion order (`sink.rs`) — the schema/README side of that drift is already corrected on `fix/bug-audit-sweep` (L29: key order is now documented as platform-dependent). Tighten the rest when the contract next evolves.
