...
- **[faces/DONE the over-merge; HIGH remaining = recall] mega-cone fixed (PR: faces-clustering-redesign).** Diagnosed the 10,690-face cone as Pass-1 hub-bridge chaining and fixed it by tightening `pass1_cosine` 0.66→0.82 (+ pass2 0.74, automerge 0.88): 44k faces-in-coherent-clusters 14%→67%, largest cone 10,690→1,586. Validated label-free via cohesion (STATE 2026-07-05). **Remaining (needs a LABELLED set to do right):** (1) the precision/recall knob — 0.82 clusters ~43% of faces; a **mutual-nearest re-merge stage** (merge two tight cores only if each is the other's nearest at high cosine) would recover recall without re-coning, letting pass1 go tighter safely; (2) the residual 1,586-face cone at cohesion 0.63; (3) full-corpus (84k) confirmation once the F:\TrueNAS scan can finish (needs `-ScanTimeoutMinutes 120`, external drive is slow). Don't push pass1 tighter as a blind default — 0.85 craters recall on sparse libraries.
