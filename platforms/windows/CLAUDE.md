...
Shipped:
- Rust engine: full scan pipeline (discovery / tagging / dbwriter); ONNX Runtime ML inference (SCRFD + ArcFace faces, MobileCLIP embeddings with CLIP zero-shot scene tagging as the canonical auto-tagger) with EP auto-select (CUDA / TensorRT / DirectML / OpenVINO / QNN / CPU) + discrete-GPU pinning + TDR (`DXGI_ERROR_DEVICE_REMOVED`) hardening; Windows.Media.Ocr; llama.cpp-subprocess VLMs (Qwen2.5-VL / Gemma) for opt-in Deep Analyze; face clustering; restructure / cleanup; v1–v7 migrations; HuggingFace model downloader; parent-PID watchdog; WAL checkpoint; structured local-only tracing.
