...
Build from source for now; refreshed downloadable CLI and TUI archives will return with the next cross-platform release. This command builds the engine, CLI, and TUI in release and installs `fileid`, `fileid-tui`, and the engine binary to `~/.cargo/bin`:
...
| :-- | :-- | :-- |
| **Windows app** | Build from source | Refreshed x64/ARM64 installers will return with the next Windows release. |
| **macOS app** | `FileID-v0.1.0.dmg` | Apple Silicon app bundle in an unsigned prerelease disk image. |
| **CLI + TUI** | Build from source | Refreshed x64/ARM64 archives will return with the next cross-platform release. |
