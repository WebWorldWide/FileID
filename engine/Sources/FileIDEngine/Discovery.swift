...
//
// Walks the directory tree under a given root, batched 1024 entries per
// FileManager.enumerator pull. After enumeration completes, sorts the result
// lexicographically by path — this gives us I/O locality on NAS volumes
// (consecutive files in the same folder are read together, exploiting the
// SMB/NFS prefetch window). Sort is O(N log N) in memory; trivial cost.
//
// Filters: skip hidden files, skip files larger than 500 MB (huge videos /
// archives where Vision tagging has nothing to say and decode can OOM on
// 16 GB), skip non-regular files (symlinks resolved by the enumerator).
//
// Output: an array of DiscoveredFile, ready to be pushed into an AsyncChannel
// at Milestone 2 when tagging comes online.
import Foundation
