// MobileCLIPService — v2 port of v1's CLIP image-encoder service.
//
// Loads Apple's MobileCLIP-S2 CoreML image encoder (the same .mlpackage v1
// already downloads to ~/Library/Application Support/FileID/Models/), runs
// 256×256 BGRA pixel buffers through it, returns L2-normalized 512-d float
// vectors.
//
// Design carried forward from v1's hard-won lessons:
//  - Double-checked locking on load (Batch 17/18 fix). Without it, N
//    concurrent first callers would race in compileAndLoad and thrash ANE
//    with N parallel ~100 MB MLModel(contentsOf:) compiles. We preserve the
//    fix verbatim AND add an explicit pre-warm at scan start (Tagging.swift)
//    so the lock is never contended in production.
//  - `.computeUnits = .all` so CoreML routes to ANE/GPU/CPU as available.
//  - 256×256 input — MobileCLIP-S2's expected size, hardcoded. Larger inputs
//    waste cycles without improving accuracy.
//
// Concurrency: the SERVICE is `@unchecked Sendable` (locks guard internals).
// The CALLER (Tagging stage) is responsible for the ANE concurrency bound —
// flooding embed() from 14 workers thrashes the ANE; we use an external
// AsyncSemaphore to cap in-flight to ~2 (per the plan).
import Foundation
import CoreML
...
public final class MobileCLIPService: @unchecked Sendable {
    ...
    public func loadImageEncoder(at url: URL? = nil) -> Bool {
        ...
        guard FileManager.default.fileExists(atPath: modelURL.path) else {
            JSONLog.shared.warn(ev: "clip_model_missing", path: modelURL.path,
                                error: "MobileCLIP-S2 image weights not downloaded; embeddings disabled")
            return false
        }
        let config = MLModelConfiguration()
        config.computeUnits = .all
        // .mlpackage needs to be compiled first into a .mlmodelc cache. CoreML
        // does this transparently on first MLModel(contentsOf:), but the cache
        // location matters — Application Support is fine, /tmp can fail under
        // sandboxing. We compile explicitly and load the compiled .mlmodelc.
        let model: MLModel
        do {
            let compiledURL = try MLModel.compileModel(at: modelURL)
            model = try MLModel(contentsOf: compiledURL, configuration: config)
        } catch {
            JSONLog.shared.error(ev: "clip_model_load_failed", path: modelURL.path,
                                 error: "\(error)")
            return false
        }
        lock.lock()
        ...
        isImageLoaded = true
        lock.unlock()
        JSONLog.shared.info(ev: "clip_model_loaded", path: modelURL.path)
        return true
    }
