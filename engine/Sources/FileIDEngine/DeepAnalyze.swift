// DeepAnalyze — local VLM inference (Qwen / Gemma / SmolVLM / PaliGemma)
// via Apple MLX. Used on demand from the UI: per file, per folder, or
// across the entire library.
//
// Same architectural shape as v1's DeepAnalyzeService, simplified for
// the engine's process-isolated context. Works while the system is
// asleep / lid-closed (when paired with the SleepGuard assertion).
//
// The service caches the loaded ModelContainer between calls so a batch
// pass doesn't re-download / re-load weights per file. Switching models
